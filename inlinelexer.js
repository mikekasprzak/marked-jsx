...
export default class InlineLexer {
	...
	output( src ) {
		...
		while ( src ) {
			...
			if ( cap = this.rules.tag.exec(src) ) {
				...
				src = src.substring(cap[0].length);
				out.push(this.options.sanitize ? (this.options.sanitizer ? this.options.sanitizer(cap[0]) : Util.escape(cap[0])) : cap[0]);
				continue;
			}
	...
	*/
	outputLink( cap, link ) {
		...
		title = link.title ? Util.escape(link.title) : null;

		return (cap[0].charAt(0) !== '!') ? this.renderer.link(href, title, this.output(cap[1])) : this.renderer.image(href, title, Util.escape(cap[1]));
	}
